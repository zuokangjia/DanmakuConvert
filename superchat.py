...
class SuperChat:
    def __init__(self, appear_time, disappear_time, user_name, price, btm_box_height, position_y, message,
                msg_box_position_x=20, msg_box_position_y=0, msg_space_x=500, msg_space_y=1080, sc_font_size=38):
        """
        Args:
    ...
    def draw_upper_box(self, upper_box_color):
        """
        ...
        """
        start_time = format_time(self.appear_time)
        end_time = format_time(self.disappear_time)
        effect = f"\\pos(20,{self.position_y})"
    ...
    def draw_user_name(self, user_name_color):
        """
        ...
        start_time = format_time(self.appear_time)
        end_time = format_time(self.disappear_time)
        position_user_name_y = self.position_y + int(self.sc_font_size / 6)
        effect = f"\\pos(20,{position_user_name_y})"
    ...
    def draw_superchat_price(self):
        """
        Draw the price of the superchat.
        """
        start_time = format_time(self.appear_time)
        end_time = format_time(self.disappear_time)
        position_superchat_price_y = self.position_y + int(self.sc_font_size / 6) + self.sc_font_size
        effect = f"\\pos(20,{position_superchat_price_y})"
    ...
    def draw_lower_box(self, lower_box_color):
        """
        ...
        start_time = format_time(self.appear_time)
        end_time = format_time(self.disappear_time)
        position_lower_box_y = self.position_y + math.ceil(self.sc_font_size + self.sc_font_size*(4.0/5.0) + self.radius/2)
        effect = f"\\pos(20,{position_lower_box_y})"
        lower_box = (
    ...
    def draw_superchat_message(self):
        """
    ...
    def write_superchat(self, ass_path):
        ...
        with open(ass_path, "a") as f:
            f.write(self.draw_upper_box(upper_box_color,) + "\n")
            f.write(self.draw_lower_box(lower_box_color) + "\n")
            f.write(self.draw_user_name(user_name_color) + "\n")
            f.write(self.draw_superchat_price() + "\n")
            f.write(self.draw_superchat_message() + "\n")